...
class AG_NEWS_DATASET:
    ...
    def load_data(self, distributed=False, num_workers=2):
        ...
        self.train_ds = torch.utils.data.Subset(self.train_ds, range(0, len(self.train_ds), 10))
        self.test_ds = torch.utils.data.Subset(self.test_ds, range(0, len(self.test_ds), 10))
...
def train(model, criterion, alpha, ngd, rank=0):
    ...
    ##
    if ngd:
        optimizer = NGD(model.parameters(), lr=args.lr, weight_decay=args.weight_decay, momentum=0.)
    else:
        optimizer = SGD(model.parameters(), lr=args.lr, weight_decay=args.weight_decay, momentum=0.)
...
def parse():
    ...
    parser.add_argument("--weight_decay", help="set weight decay of optimizer", default=5e-4, type=float)
